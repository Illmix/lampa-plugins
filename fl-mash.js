...
(function () {
    ...
    function rezka2(component, _object) {
      ...
      function append(items) {
        component.reset();
        component.drawPrestige(items, {
          source_id: 'rezka2',
          source_name: 'HDrezka',
          voice_name: filter_items.voice[choice.voice] || '',
          onEnter: function (element, html) {
            if (element.loading) return;
            element.loading = true;
            getStream(element, function (element) {
              element.loading = false;
          ...
          onContextFile: function (element, call) {
            getStream(element, function (element) {
              call({
                file: element.stream,
                quality: element.qualitys
    ...
    function component(object) {
      ...
      this.similars = function (json, search_more, more_params) {
        ...
        json.forEach(function (elem) {
          ...
          var year_str = year ? (year + '').slice(0, 4) : '';
          var poster = elem.img || elem.poster || elem.poster_url || elem.posterUrl || elem.picture || elem.thumb || elem.thumbnail || elem.image || elem.preview || '';
          var info = [];
          if (year_str) info.push(year_str);
          if (orig_title && orig_title != title) info.push(orig_title);
          ...
          elem.title = title;
          var item = Lampa.Template.get('online_mod_prestige_folder', {
            title: title,
            time: '',
          ...
          if (poster) {
            var image = $('<img style="height: 7em; width: 7em; border-radius: 0.3em; object-fit: cover;"/>');
            item.find('.online-prestige__folder').empty().append(image);

            if (Lampa.Utils && Lampa.Utils.imgLoad) {
              Lampa.Utils.imgLoad(image, poster);
            } else {
              image[0].onerror = function () {
                image[0].src = './img/img_broken.svg';
              };
              image[0].src = poster;
            }
          }
        ...
        if (search_more) {
          ...
          var item = Lampa.Template.get('online_mod_prestige_folder', elem);
          item.on('hover:enter', function () {
            _this5.activity.loader(true);
        ...
        params.item.on('hover:long', function () {
          ...
          Lampa.Select.show({
            ...
            onSelect: function onSelect(a) {
              function appendViewed(p) {
                if (p.prestige) {
                  p.item.find('.online-prestige__img').append('<div class="online-prestige__viewed">' + Lampa.Template.get('icon_star', {}, true) + '</div>');
                } else {
                  p.item.append('<div class="torrent-item__viewed">' + Lampa.Template.get('icon_star', {}, true) + '</div>');
                }
              }

              function removeViewed(p) {
                p.item.find(p.prestige ? '.online-prestige__viewed' : '.torrent-item__viewed').remove();
              }
              ...
              if (a.clearmark_all) {
                contextmenu_all.forEach(function (p) {
                  Lampa.Arrays.remove(p.viewed, p.hash_file);
                  Lampa.Storage.set('online_view', p.viewed);
                  removeViewed(p);
                });
              }
      ...
      this.clearPrestigeImages = function () {
        prestige_images.forEach(function (img) {
      ...
      var prestige_network = new Lampa.Reguest();

      this.getEpisodes = function (season, call) {
        var episodes = [];
        var movie = object.movie;
        var tmdb_id = movie.id;
        if (['cub', 'tmdb'].indexOf(movie.source || 'tmdb') == -1) tmdb_id = movie.tmdb_id;

        if (typeof tmdb_id == 'number' && movie.name) {
          ...
          prestige_network.timeout(1000 * 10);
          prestige_network["native"](baseurl, function (data) {
      ...
      this.watched = function (set) {
        var movie = object.movie;
        var file_id = Lampa.Utils.hash(movie.number_of_seasons ? movie.original_name : movie.original_title);
      ...
      this.updateWatched = function () {
        ...
        var body = scroll.render().find('.online-prestige-watched .online-prestige-watched__body').empty();
        if (!body.length) return;

        if (watched) {
          var line = [];
          if (watched.source_name) line.push(watched.source_name);
          ...
          body.append('<span>' + Lampa.Lang.translate('online_mod_no_watch_history') + '</span>');
        }
      };

      this.drawPrestige = function (items, params) {
        var self = this;
        params = params || {};
        if (!items.length) return;

        self.clearPrestigeImages();

        var movie = object.movie;
        var serial = !!movie.name;
        var fully = window.innerWidth > 480;
        var voice_name_default = params.voice_name || '';
        var source_name = params.source_name || '';
        var hash_file_for = params.hash_file_for || function (element) {
          return Lampa.Utils.hash(element.season ? [element.season, element.season > 10 ? ':' : '', element.episode, movie.original_title, voice_name_default || element.voice_name || element.title].join('') : movie.original_title + (voice_name_default || element.voice_name || element.title));
        };

        if (params.show_watched !== false) {
          scroll.append(Lampa.Template.get('online_mod_prestige_watched', {}));
          self.updateWatched();
        }

        self.getEpisodes(items[0] && items[0].season, function (episodes) {
          var viewed = Lampa.Storage.cache('online_view', 5000, []);
          var scroll_to_mark = false;

          items.forEach(function (element, index) {
            var episode_num = element.episode || index + 1;
            var voice_name = voice_name_default || element.voice_name || (serial ? element.title || 'Unknown' : element.title) || 'Unknown';
            element.voice_name = voice_name;
            element.translate_voice = voice_name;
            element.translate_episode_end = serial ? self.getLastEpisode(items) : undefined;

            var episode = serial && episodes.length && !params.similars ? episodes.find(function (e) {
              return e.episode_number == element.episode;
            }) : false;

            var hash = Lampa.Utils.hash(element.season ? [element.season, element.season > 10 ? ':' : '', element.episode, movie.original_title].join('') : movie.original_title);
            var hash_file = hash_file_for(element);
            var view = Lampa.Timeline.view(hash);
            element.timeline = view;

            var info_html_parts = [];
            var voice_short = voice_name.length > 60 ? voice_name.substr(0, 60) + '...' : voice_name;
            var title = element.title || '';

            if (episode) {
              title = episode.name || title;
              if (voice_short.length < 30 && episode.vote_average) {
                info_html_parts.push(Lampa.Template.get('online_mod_prestige_rate', {
                  rate: parseFloat(episode.vote_average + '').toFixed(1)
                }, true));
              }
              if (episode.air_date && fully) info_html_parts.push(Lampa.Utils.parseTime(episode.air_date).full);
            } else if (movie.release_date && fully) {
              info_html_parts.push(Lampa.Utils.parseTime(movie.release_date).full);
            }

            if (!serial && movie.tagline && voice_short.length < 30) info_html_parts.push(movie.tagline);
            if (voice_short) info_html_parts.push(voice_short);

            var info_html = info_html_parts.length ? info_html_parts.map(function (i) {
              return '<span>' + i + '</span>';
            }).join('<span class="online-prestige-split">●</span>') : '';

            var runtime = (episode ? episode.runtime : movie.runtime) || 0;
            var time_str = runtime ? Lampa.Utils.secondsToTime(runtime * 60, true) : '';

            var html = Lampa.Template.get('online_mod_prestige_full', {
              title: title,
              time: time_str,
              info: info_html,
              quality: ''
            ...
            if (serial && !episode) {
              ...
              loader.remove();
            } else if (!serial && (!movie.backdrop_path || movie.backdrop_path == 'undefined')) {
              loader.remove();
            } else {
              ...
              img.src = Lampa.TMDB.image('t/p/w300' + (episode ? episode.still_path : movie.backdrop_path));
              prestige_images.push(img);
            }
            ...
            if (viewed.indexOf(hash_file) !== -1) {
              scroll_to_mark = html;
              image.append('<div class="online-prestige__viewed">' + Lampa.Template.get('icon_star', {}, true) + '</div>');
            }

            element.mark = function () {
              var v = Lampa.Storage.cache('online_view', 5000, []);
              if (v.indexOf(hash_file) == -1) {
                v.push(hash_file);
                Lampa.Storage.set('online_view', v);
                if (image.find('.online-prestige__viewed').length == 0) {
                  image.append('<div class="online-prestige__viewed">' + Lampa.Template.get('icon_star', {}, true) + '</div>');
                }
              }
              var voice_short_w = voice_name.length > 30 ? voice_name.slice(0, 30) + '...' : voice_name;
              self.watched({
                source: params.source_id,
                source_name: source_name,
                voice_name: voice_short_w,
            ...
            element.unmark = function () {
              var v = Lampa.Storage.cache('online_view', 5000, []);
              if (v.indexOf(hash_file) !== -1) {
                Lampa.Arrays.remove(v, hash_file);
                Lampa.Storage.set('online_view', v);
                image.find('.online-prestige__viewed').remove();
              }
            };
            ...
            self.contextmenu({
              ...
              prestige: true,
              file: function file(call) {
                if (params.onContextFile) params.onContextFile(element, call);
              }
            });
          });
          ...
          if (serial && episodes.length > items.length && !params.similars) {
            ...
            var s_num = items[0] ? items[0].season : 1;
            left.forEach(function (ep) {
              var einfo = [];
              if (ep.vote_average) einfo.push(Lampa.Template.get('online_mod_prestige_rate', {
                rate: parseFloat(ep.vote_average + '').toFixed(1)
              }, true));
              if (ep.air_date) einfo.push(Lampa.Utils.parseTime(ep.air_date).full);
              var air = new Date((ep.air_date + '').replace(/-/g, '/'));
              var now = Date.now();
              var day = Math.round((air.getTime() - now) / (24 * 60 * 60 * 1000));
              var txt = Lampa.Lang.translate('full_episode_days_left') + ': ' + day;
              var ehtml = Lampa.Template.get('online_mod_prestige_full', {
                time: Lampa.Utils.secondsToTime((ep.runtime || movie.runtime || 0) * 60, true),
                info: einfo.length ? einfo.map(function (i) {
                  return '<span>' + i + '</span>';
                }).join('<span class="online-prestige-split">●</span>') : '',
                title: ep.name,
                quality: day > 0 ? txt : ''
              });
              var eloader = ehtml.find('.online-prestige__loader');
              var eimage = ehtml.find('.online-prestige__img');
              ehtml.find('.online-prestige__timeline').append(Lampa.Timeline.render(Lampa.Timeline.view(Lampa.Utils.hash([s_num, ep.episode_number, movie.original_title].join('')))));

              if (ep.still_path) {
                var eimg = ehtml.find('img')[0];
                eimg.onerror = function () {
                  eimg.src = './img/img_broken.svg';
                };
                eimg.onload = function () {
                  eimage.addClass('online-prestige__img--loaded');
                  eloader.remove();
                  eimage.append('<div class="online-prestige__episode-number">' + ('0' + ep.episode_number).slice(-2) + '</div>');
                };
                eimg.src = Lampa.TMDB.image('t/p/w300' + ep.still_path);
                prestige_images.push(eimg);
              } else {
                eloader.remove();
                eimage.append('<div class="online-prestige__episode-number">' + ('0' + ep.episode_number).slice(-2) + '</div>');
              }

              ehtml.css('opacity', '0.5');
              self.append(ehtml);
            });
          }

          if (scroll_to_mark) self.setLast(scroll_to_mark[0]);
      ...
      this.start = function (first_select) {
        ...
        if (first_select) {
          var prestige_items = scroll.render().find('.online-prestige.selector').not('.online-prestige-watched');
          if (prestige_items.length) {
            var prestige_views = prestige_items.find('.online-prestige__viewed').closest('.online-prestige.selector');
            if (object.movie.number_of_seasons && prestige_views.length) last = prestige_views.last()[0];else last = prestige_items.eq(0)[0];
          } else {
    ...
    function initStorage() {
      if (!Utils.isDebug()) {
        ...
        Lampa.Storage.set('online_mod_proxy_rezka2_mirror', true);
        Lampa.Storage.set('online_mod_proxy_kinobase', Lampa.Platform.is('android') || isLocal ? 'false' : 'true');
