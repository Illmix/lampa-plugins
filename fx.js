  ...
  function component(object) {
    ...
    this.similars = function(json) {
      ...
      json.forEach(function(elem) {
        // Логика формирования инфо-строки как в vod.js
        var info = [];
        ...
        if (elem.rating && elem.rating !== 'null') {
          info.push(Lampa.Template.get('online_prestige_rate', {
            rate: parseFloat(elem.rating).toFixed(1)
          }, true));
        }
        if (year) info.push(year);
        if (elem.details) info.push(elem.details);
        else if (elem.countries && elem.countries.length) {
          // Фоллбэк для Filmix данных
          info.push((elem.filmId ? elem.countries.map(function(c) {
            return c.country;
          }) : elem.countries).join(', '));
        }

        var name = elem.title || elem.ru_title || elem.en_title || elem.nameRu || elem.nameEn || elem.text;
        elem.title = name;
        elem.time = elem.time || elem.filmLength || '';
        elem.info = info.join('<span class="online-prestige-split">●</span>');

        var item = Lampa.Template.get('online_prestige_folder', elem);

        // === ВАЖНО: Добавлена логика отображения картинок для сезонов/похожих (из vod.js) ===
        var img_url = elem.img || elem.poster; // Filmix часто использует poster
        if (img_url) {
          ...
          if (img_url.indexOf('http') == -1) {
            // Обработка относительных путей, если нужно (как в vod.js)
            if (img_url.charAt(0) === '/') img_url = 'http://filmixapp.vip' + img_url;
          }
      ...
      this.getEpisodes(items[0].season, function(episodes) {
        ...
        items.forEach(function(element, index) {
          ...
          element.mark = function() {
            ...
            if (voice_name_text && voice_name_text.length > 30) voice_name_text = voice_name_text.slice(0, 30) + '...';
